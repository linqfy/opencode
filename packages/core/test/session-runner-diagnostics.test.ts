...
import { StepUsageTable } from "@opencode-ai/core/capability/sql"
...
const sessionID = SessionV2.ID.make("ses_runner_diagnostics")
  ...
  it.effect("records one step_usage row with the resolved profile id", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* db
        .select()
        .from(StepUsageTable)
        .where(eq(StepUsageTable.session_id, sessionID))
        .all()
        .pipe(Effect.orDie)
