import type { PlannerMessage, PlannerPart, PlannerRole } from "@ultracode/context"
...
const roleFor = (type: SessionMessage.Message["type"]): PlannerRole | undefined => {
  switch (type) {
    case "user":
    case "synthetic":
    case "shell":
...
const textPart = (id: string, text: string, over: Partial<PlannerPart> = {}): PlannerPart => ({
  id,
  kind: "text",
  text,
  tokens: tokens(text),
...
const partsFor = (message: SessionMessage.Message): PlannerPart[] => {
  ...
  return message.content.flatMap((part): PlannerPart[] => {
    ...
    const call: PlannerPart = { id: part.id, kind: "tool_use", toolName: part.name, toolCallId: part.id, text: input, tokens: tokens(input) }
    if (part.state.status === "completed") {
      const output = part.state.content.map((c) => (c.type === "text" ? c.text : `[Attached ${c.mime}]`)).join("\n")
      const result: PlannerPart = { id: `${part.id}-result`, kind: "tool_result", toolName: part.name, toolCallId: part.id, text: output, tokens: tokens(output) }
      return [call, result]
    }
    if (part.state.status === "error") {
      const output = part.state.error.message
      const result: PlannerPart = { id: `${part.id}-result`, kind: "tool_result", toolName: part.name, toolCallId: part.id, text: output, tokens: tokens(output), activeFailure: true }
      return [call, result]
    }
    return [call]
  })
...
export const toPlannerMessages = (entries: readonly Entry[]): PlannerMessage[] => {
  const result: PlannerMessage[] = []
